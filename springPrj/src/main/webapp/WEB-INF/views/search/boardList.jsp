                                            ...
                                            <c:when test="${fn:length(list) > 0 }">
                                                <c:forEach items="${list }" var="bList">
                                                    <tr>
                                                        ...
                                                        <td class="mailbox-subject"><a href='<c:url value='/search/boardDetail${pageMaker.makeQuery(pageMaker.cri.page) }&IDX=${bList.IDX }'/>'>${bList.TITLE }</a></td>
                            ...
                            <div class="mailbox-controls">
                                ...
                                <div class="pull-right">
	                                <ul class="btn-group pagination" style="margin:0">
	                                    ...
	                                    <li>
	                                        <a href='<c:url value="/search/boardList${pageMaker.makeQuery(pageMaker.startPage-1) }"/>' role="button" class="btn btn-default btn-sm"><i class="fa fa-chevron-left"></i></a>
	                                    </li>
	                                    </c:if>
	                                    <c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="idx">
	                                    <li <c:out value="${pageMaker.cri.page==idx ? 'class=active' : '' }" />>
	                                        <a href='<c:url value="/search/boardList${pageMaker.makeQuery(idx) }"/>' role="button" class="btn btn-default btn-sm"><i class="fa">${idx }</i></a>
	                                    </li>
	                                    </c:forEach>
	                                    <c:if test="${pageMaker.next && pageMaker.endPage >0 }">
	                                    <li>
	                                        <a href='<c:url value="/search/boardList${pageMaker.makeQuery(pageMaker.endPage+1) }"/>' role="button" class="btn btn-default btn-sm"><i class="fa fa-chevron-right"></i></a>
	                                    </li>
	                                    </c:if>
	                                </ul>
                                    <!-- /.btn-group -->
