                    ...
                    <div class="box box-primary">
                        ...
                        <div class="box-footer no-padding">
                            <div class="mailbox-controls">
                                ...
                                <a href='<c:url value="/reply/boardWrite"/>' role="button" class="btn btn-default btn-sm checkbox-toggle"><i class="fa fa-edit"> 글작성</i></a>
                                </sec:authorize>
                                <div class="btn-group">
                                ...
                                <div class="pull-right">
	                                <ul class="btn-group pagination" style="margin:0">
	                                    ...
	                                    <li>
	                                        <a href='<c:url value="/reply/boardList${pageMaker.makeSearch(pageMaker.startPage-1) }"/>' role="button" class="btn btn-default btn-sm"><i class="fa fa-chevron-left"></i></a>
	                                    </li>
	                                    </c:if>
	                                    <c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="idx">
	                                    <li <c:out value="${pageMaker.cri.page==idx ? 'class=active' : '' }" />>
	                                        <a href='<c:url value="/reply/boardList${pageMaker.makeSearch(idx) }"/>' role="button" class="btn btn-default btn-sm"><i class="fa">${idx }</i></a>
	                                    </li>
	                                    </c:forEach>
	                                    <c:if test="${pageMaker.next && pageMaker.endPage >0 }">
	                                    <li>
	                                        <a href='<c:url value="/reply/boardList${pageMaker.makeSearch(pageMaker.endPage+1) }"/>' role="button" class="btn btn-default btn-sm"><i class="fa fa-chevron-right"></i></a>
	                                    </li>
	                                    </c:if>
	                                </ul>
                                    <!-- /.btn-group -->
	...
	<script type="text/javascript">
		$(function(){
			$("#searchBtn").on("click", function(event){
				...
				self.location = "${pageContext.request.contextPath}/reply/boardList${pageMaker.makeQuery(1)}&searchType="+searchType+"&keyword="+keyword;
			});
		});
	</script>
